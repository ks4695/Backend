...
const CONFIG = {
  ...
  // ✨ n8n 경유 Microsoft 연동 (2026-09 도입) — CLIENT_ID/CLIENT_SECRET/REFRESH_TOKEN 직접 관리 대신
  //    n8n에 등록된 Microsoft credential을 통해 OneDrive 파일 읽기/쓰기 및 메일 발송을 위임한다.
  //    → 나중에 로그인 계정(개인→회사 계정 등)이 바뀌어도, n8n의 Credential만 교체하면 되고
  //       이 백엔드 코드는 전혀 건드릴 필요가 없다.
  n8nDownloadUrl: process.env.N8N_DOWNLOAD_URL,   // 예: https://workflow-edu.kolmar.co.kr/webhook/download-excel
  n8nUploadUrl: process.env.N8N_UPLOAD_URL,       // 예: https://workflow-edu.kolmar.co.kr/webhook/upload-excel
  n8nSendMailUrl: process.env.N8N_SENDMAIL_URL,   // 예: https://workflow-edu.kolmar.co.kr/webhook/send-mail
  n8nHeaderName: process.env.N8N_HEADER_NAME || 'X-Internal-Key',
  n8nHeaderValue: process.env.N8N_HEADER_VALUE     // n8n Webhook의 Header Auth에 설정한 그 비밀값
};
...
console.log(`   n8n Download URL: ${CONFIG.n8nDownloadUrl ? '✅ 설정됨' : '❌ 미설정'}`);
console.log(`   n8n Upload URL: ${CONFIG.n8nUploadUrl ? '✅ 설정됨' : '❌ 미설정'}`);
console.log(`   n8n SendMail URL: ${CONFIG.n8nSendMailUrl ? '✅ 설정됨' : '❌ 미설정'}`);
console.log(`   n8n Header Value: ${CONFIG.n8nHeaderValue ? '✅ 설정됨' : '❌ 미설정 (Webhook이 잠겨있으면 모든 요청 실패함)'}`);
...
// ✨ (2026-09~) 메일 발송을 n8n 경유로 위임. n8n에 등록된 Microsoft Outlook credential이
//    실제 발신 계정을 담당하므로, 나중에 발신 계정(개인→회사 등)이 바뀌어도 n8n의 Credential만
//    교체하면 되고 이 코드는 그대로 둔다. n8n 워크플로우 쪽 구성: Webhook(POST, Header Auth)
//    → Microsoft Outlook 노드(또는 HTTP Request로 /me/sendMail 호출) → Respond to Webhook.
async function sendVerificationEmail(toEmail, code) {
  if (!CONFIG.n8nSendMailUrl) {
    throw new Error('N8N_SENDMAIL_URL 환경변수가 설정되지 않았습니다.');
  }
  ...
  await axios.post(
    ...
    {
      headers: {
        [CONFIG.n8nHeaderName]: CONFIG.n8nHeaderValue,
...
async function fetchExcelFromOneDrive() {
  ...
  try {
    if (!CONFIG.n8nDownloadUrl) {
      throw new Error('N8N_DOWNLOAD_URL 환경변수가 설정되지 않았습니다.');
    }
    console.log(`📥 n8n 경유로 "${CONFIG.excelFileName}" 다운로드 중...`);

    const response = await axios.get(
      CONFIG.n8nDownloadUrl,
      {
        headers: { [CONFIG.n8nHeaderName]: CONFIG.n8nHeaderValue },
...
async function updateExcelOnOneDrive(data, retries = 3) {
  ...
  if (!CONFIG.n8nUploadUrl) {
    console.error('❌ N8N_UPLOAD_URL 환경변수가 설정되지 않았습니다. 저장을 진행할 수 없습니다.');
    return false;
  }
