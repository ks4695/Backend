...
const CONFIG = {
  ...
  // ✨ (되돌림) n8n 경유 Microsoft 연동 시도는 폐기하고, 기존 방식대로
  //    CLIENT_ID/CLIENT_SECRET/REFRESH_TOKEN을 이용한 Graph API 직접 호출로 복원했다.
  //    (OneDrive 파일 읽기/쓰기, 인증코드 메일 발송 모두 getValidAccessToken()의
  //     access_token으로 Graph API를 직접 호출한다.)
};
...
console.log(`   JWT Secret: ${process.env.JWT_SECRET ? '✅ 설정됨' : '❌ 미설정 — 반드시 설정 필요! 없으면 서버 재시작마다 모든 로그인이 풀립니다'}`);

// ✨ (2026-09~) 로그인 세션을 파일(sessions.json)에 저장하는 대신, 서명된 JWT로 완전히
//    무상태(stateless)화했다. Render 무료 플랜은 15분 이상 유휴 상태면 서버를 재웠다가
//    깨우는데, 이때 디스크가 초기화되어 sessions.json이 통째로 사라지는 문제가 있었다.
//    JWT 방식은 서버가 아무것도 "기억"할 필요가 없어(서명 검증만 하면 됨) 이 문제가
//    원천적으로 사라진다. JWT_SECRET은 반드시 Render 환경변수로 고정값을 넣어야 하며,
//    없으면 서버가 뜰 때마다 다른 값으로 서명하게 되어 역시 모든 세션이 무효화된다.
const JWT_SECRET = process.env.JWT_SECRET;
if (!JWT_SECRET) {
  console.error('🚨 JWT_SECRET 환경변수가 없습니다! 로그인 세션이 서버 재시작마다 전부 풀립니다.');
}
...
// (SESSION_FILE 제거됨 — 세션은 이제 JWT로 무상태 처리)
...
// ✨ 메일 발송: OneDrive와 동일한 Azure 앱(CLIENT_ID/REFRESH_TOKEN) 계정으로
//    Graph API /me/sendMail을 직접 호출한다 (n8n 경유 방식은 폐기).
...
// ✨ 세션 = 서명된 JWT. 서버는 아무 상태도 저장하지 않고, 토큰의 서명이 유효한지와
//    만료 여부만 검증한다. 그래서 서버가 재시작되어도(디스크 초기화 포함) 클라이언트가
//    들고 있는 토큰은 계속 유효하게 검증된다.
function createSession(email, name) {
  return jwt.sign({ email, name }, JWT_SECRET, { expiresIn: SESSION_DURATION_MS / 1000 });
}
...
function getSession(token) {
  ...
  try {
    ...
    // 만료됐거나(TokenExpiredError), 서명이 안 맞거나(JsonWebTokenError) — 둘 다 로그아웃 처리
    return null;
  }
}

// JWT는 무상태라 서버가 강제로 무효화할 방법이 없다 (블랙리스트를 따로 두지 않는 한).
// 로그아웃은 클라이언트가 저장해둔 토큰을 지우는 것만으로 충분하므로, 서버는 별도 처리가 필요 없다.
...
// ============================================================
// ✨ 로그 롤백 헬퍼 — updateExcelOnOneDrive(엑셀 저장) 실패 시,
//    이미 메모리(memoryLogs/facilityLogs)에 추가해둔 이번 요청의 로그를 되돌린다.
//    (저장이 실패했는데 로그만 메모리에 남아있으면, 사용자가 재시도했을 때
//     같은 이벤트가 로그에 중복으로 쌓이는 문제가 생기므로 반드시 되돌려야 한다)
// ============================================================
function removeLogsById(sharedId) {
  if (!sharedId) return;
  const generalLogs = loadLogs();
  const filtered = generalLogs.filter(l => l.id !== sharedId);
  if (filtered.length !== generalLogs.length) saveLogs(filtered);

  const idx = facilityLogs.findIndex(l => l.id === sharedId);
  if (idx !== -1) facilityLogs.splice(idx, 1);
}

// sharedId 없이 addLog만 호출한 경우(add-part/delete-part)를 위한 대체 롤백 —
// 방금 그 부품(모델명+부품종류)으로 남긴 "가장 최근" 로그 1건만 제거한다.
function removeLastLogFor(모델명, 부품종류) {
  const generalLogs = loadLogs();
  const idx = generalLogs.findIndex(l => l.모델명 === 모델명 && l.부품종류 === 부품종류);
  if (idx !== -1) {
    generalLogs.splice(idx, 1);
    saveLogs(generalLogs);
  }
}
...
app.post('/api/inventory/update', requireAuth, async (req, res) => {
  try {
    ...
    // ✨ [순서 수정] 로그를 먼저 메모리에 추가한 뒤 엑셀에 저장해야, 이번 요청의 로그가
    //    "이번" 저장에 함께 실린다. (예전 순서: 저장 → 로그 추가 였는데, 이러면 이번 로그는
    //    다음번 누군가 또 저장할 때까지 메모리에서만 대기하다가, 그 사이 서버가 재시작되면
    //    그대로 유실되는 문제가 있었다 — 실제로 9월 이력 다수가 이렇게 유실됨)
    ...
    if (success) {
      ...
      // 저장 실패 — 방금 추가한 로그를 되돌려 중복 기록을 방지 (재시도 시 다시 정상 추가됨)
      removeLogsById(sharedLogId);
      item.현재수량 = oldQuantity;
...
app.post('/api/inventory/manual-update', requireAuth, async (req, res) => {
  try {
    ...
    // ✨ [순서 수정] 로그 먼저 추가 → 그 다음 엑셀 저장 (사유는 /update 엔드포인트 주석 참고)
    const sharedLogId = uuidv4();
...
app.post('/api/inventory/add-part', requireAuth, async (req, res) => {
  try {
    ...
    if (success) {
      ...
      removeLastLogFor(newItem.모델명, newItem.부품종류);
      return res.status(500).json({ success: false, message: 'OneDrive 업데이트 실패' });
    }
  } catch (error) {
...
app.post('/api/inventory/delete-part', requireAuth, async (req, res) => {
  try {
    ...
    if (success) {
      ...
      removeLastLogFor(deletedItem.모델명, deletedItem.부품종류);
      return res.status(500).json({ success: false, message: 'OneDrive 업데이트 실패' });
    }
  } catch (error) {
...
app.get('/api/server-status', (req, res) => {
  ...
  res.json({
    ...
    localFilePersistence: {
      ...
      user_auth: checkFile(USERS_FILE),
      sessions: 'stateless (JWT) — 파일 없음, 서버 재시작과 무관하게 유지됨'
    }
  });
});
...
~~~`;
    ...
    // ✨ Gemini 무료 플랜은 회사 전체가 같은 API 키를 공유해서 쓰기 때문에,
    //    "분당 요청 수" 한도가 한 사람이 아니라 전체 사용자 합산으로 깎인다.
    //    → 동시에 여러 명이 쓰면 개개인 기준으로는 조금만 써도 429(Too Many Requests)가 자주 뜬다.
    //    아래에서는 (1) 429/503처럼 "잠깐 후 재시도하면 될 수도 있는" 에러는 짧게 한 번
    //    자동 재시도하고, (2) 그래도 실패하면 원인을 사용자가 알아볼 수 있는 말로 안내한다.
    const callGemini = () => model.generateContent({
      contents,
    ...
    try {
      try {
        ...
        // 429(사용량 초과) / 503(모델 과부하)만 짧게 재시도. 그 외(400, 403 등)는 재시도해도 똑같이 실패하므로 바로 포기.
        if (status === 429 || status === 503) {
          console.warn(`⚠️ Gemini ${status} 발생 — 4초 후 1회 재시도`);
          await wait(4000);
          result = await callGemini();
        } else {
          throw firstError;
      ...
      console.error(`❌ Gemini API 호출 실패 (status: ${status ?? '없음'}):`, apiError.message);
      console.error('❌ Gemini API 에러 상세:', JSON.stringify(apiError?.response?.data || apiError?.errorDetails || apiError, null, 2).slice(0, 2000));

      let userMessage;
      if (status === 429) {
        userMessage = '⚠️ 지금 AI 사용량이 몰려서 잠시 응답을 드릴 수 없어요 (무료 요금제 분당 사용량 한도 초과 — 여러 사람이 동시에 써서 그럴 수 있어요). 1분 정도 후 다시 시도해 주세요.';
      } else if (status === 503) {
        userMessage = '⚠️ 지금 AI 모델이 일시적으로 응답이 몰려있어요 (구글 서버 과부하). 잠시 후 다시 시도해 주세요.';
      } else if (status === 500) {
        userMessage = '⚠️ AI 서비스 쪽에서 일시적인 오류가 발생했어요 (구글 서버 내부 오류). 잠시 후 다시 시도해 주세요.';
      } else if (status === 400) {
        userMessage = '⚠️ 요청 형식에 문제가 있어 AI가 응답하지 못했어요. 문장을 조금 짧게 바꿔서 다시 말씀해 주시겠어요? (계속되면 관리자에게 알려주세요)';
      } else if (status === 403) {
        userMessage = '⚠️ AI 서비스 접근 권한에 문제가 있어요 (API 키 설정 문제일 수 있습니다). 관리자에게 알려주세요.';
      } else if (apiError.message?.includes('ETIMEDOUT') || apiError.message?.includes('ENOTFOUND') || apiError.message?.includes('ECONNRESET')) {
        userMessage = '⚠️ AI 서버와의 네트워크 연결이 원활하지 않아요. 잠시 후 다시 시도해 주세요.';
      } else {
        userMessage = `⚠️ AI 응답을 받아오는 중 문제가 발생했습니다 (${status ? `코드: ${status}` : '알 수 없는 오류'}). 잠시 후 다시 시도해 주세요.`;
      }
